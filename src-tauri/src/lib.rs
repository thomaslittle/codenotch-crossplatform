...
fn set_edge(
    ...
    windows::place_notch(&app, edge, provider_count, scale, &monitor, offset_x, offset_y, compact)
}

#[tauri::command]
fn show_tooltip(app: tauri::AppHandle, edge: Edge, index: usize, scale: f64, compact: bool) -> Result<(), String> {
    windows::place_tooltip(&app, edge, index, scale, compact).map_err(|error| {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let _ = windows::place_notch(&handle, Edge::Right, 5, 1.0, "primary", 0.0, 0.0, false);
            });
            Ok(())
        })
